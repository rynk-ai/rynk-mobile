...
import type { SubChat, SubChatMessage } from './useGuestSubChats';

interface UseSubChatsReturn {
    // State
    subChats: SubChat[];
...
export function useSubChats(
    ...
    const loadSubChats = useCallback(async (conversationId: string) => {
        try {
            const response = await api.get<{ subChats?: SubChat[] }>(
                `/mobile/sub-chats?conversationId=${conversationId}`
            );
    ...
    const handleOpenSubChat = useCallback(
        async (
            ...
            try {
                ...
                // Create new sub-chat
                const response = await api.post<{ subChat?: SubChat }>(
                    '/mobile/sub-chats',
                    {
    ...
    const handleDeleteSubChat = useCallback(
        async (subChatId: string) => {
            try {
                await api.delete(`/mobile/sub-chats/${subChatId}`);
    ...
    const handleSubChatSendMessage = useCallback(
        async (content: string) => {
            ...
            try {
                ...
                await api.stream(
                    `/mobile/sub-chats/${activeSubChat.id}/message`,
                    { content },
                    (dataString: string) => {
                        if (!dataString) return;
                        try {
                            const parsed = JSON.parse(dataString);
                            // Similar streaming handling as web if the stream returns chunks directly
                ...
                // Re-fetch to guarantee sync after stream
                const response = await api.get<{ subChat?: SubChat }>(`/mobile/sub-chats/${activeSubChat.id}`);
                if (response.subChat) {
