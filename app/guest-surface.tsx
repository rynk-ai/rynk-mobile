...
export default function GuestSurfaceScreen() {
  ...
  const fetchSurface = useCallback(async () => {
    try {
      ...
      const response = await guestApi.generateSurface(
        messageId,
        params.query || '',
        params.type as any,
      ...
      if (response.success && response.surfaceState) {
        setSurfaceState(response.surfaceState);
      } else {
        setError(response.error || 'Failed to generate surface');
      }
    } catch (err) {
  ...
  const handleAnswerQuestion = (index: number, answer: string | number) => {
     // Local state update for mock interactivity until backend persistence is fully ready
     // ideally we'd send this to backend
     setSurfaceState(prev => {
         if (!prev || !prev.quiz) return prev;
         const newAnswers = { ...prev.quiz.answers, [index]: answer };
         
         // Simple scoring logic for UI update
